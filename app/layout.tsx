    ...
    <html lang="en">
      ...
      <body className="min-h-screen bg-[#fdfcf0] pt-6 md:pt-12 pb-24 px-2 md:px-8 font-serif selection:bg-[#ff9a9e] relative text-[#83B2DE] overflow-x-hidden">
        ...
        <motion.div 
          ...
          style={{ y: waveEffect }}
          /* FIXED: Added overflow-hidden here to clip the children into the border radius */
          className="relative z-30 w-full max-w-4xl mx-auto border-[3px] border-[#8b5a2b] rounded-[1.5rem] md:rounded-[2rem] shadow-[0px_10px_0px_0px_rgba(139,90,43,0.1)] bg-[#fffdf5] flex flex-col mb-10 overflow-hidden"
        >
          <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden rounded-[1.5rem] md:rounded-[2rem]">
            <div className="w-full h-full" style={{ backgroundImage: "url('/img/body2.jpg')", backgroundSize: 'cover', backgroundPosition: 'center' }} />
          </div>

          <div className="relative z-10 flex flex-col w-full h-full">
            <Header />

            <nav className="relative z-[60] hidden md:flex bg-[#f2ead3] px-6 gap-1 pt-2 border-b-[3px] border-[#8b5a2b] shrink-0 pointer-events-auto">
              {tabs.map((tab) => (
                <Link key={tab.path} href={tab.path} className="nav-link-fix block">
            ...
            {/* FIXED: Added bg-inherit and overflow-hidden to the content container */}
            <div className="flex-1 min-h-[300px] relative z-20 overflow-hidden">
                {children}
            </div>
          ...
          <div className="bg-[#fffdf5] border-[3px] border-[#8b5a2b] rounded-2xl p-3 shadow-[6px_6px_0px_0px_#8b5a2b] flex items-center gap-4 relative">
            ...
            <div className="flex flex-col min-w-[130px] max-w-[150px]">
              <div className="flex flex-col leading-tight">
                 ...
                 <span className="text-[9px] font-bold text-[#8b5a2b] opacity-80 truncate">{currentTrack.artist}</span>
              </div>
              <div className="flex items-center gap-2 mt-1">
                <button onClick={() => setTrackIndex((prev) => (prev - 1 + tracks.length) % tracks.length)} className="text-[#8b5a2b] hover:text-[#c45a5a]">
                  <SkipBack size={14} fill="currentColor" />
                </button>
                <button onClick={togglePlay} className="text-[#c45a5a] hover:scale-110">
                  {isPlaying ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" />}
                </button>
                <button onClick={() => setTrackIndex((prev) => (prev + 1) % tracks.length)} className="text-[#8b5a2b] hover:text-[#c45a5a]">
